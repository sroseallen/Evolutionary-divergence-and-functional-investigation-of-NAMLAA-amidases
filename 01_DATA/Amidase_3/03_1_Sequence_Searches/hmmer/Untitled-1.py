...
#
# HMMER 3 phmmer (REST) web service Python client using xmltramp2.
#
...
Synchronous job:
  ...
  Usage: python hmmer3_phmmer.py --email <your@email.com> [options...] <SeqFile|SeqID(s)>
...
Check status of Asynchronous job:
  Usage: python hmmer3_phmmer.py --status --jobid <jobId>

Retrieve job data:
  Use the jobid to query for the status of the job. If the job is finished,
  it also returns the results/errors.
  Usage: python hmmer3_phmmer.py --polljob --jobid <jobId> [--outfile string]
...
# Submit job
    ...
    if options.popen:
        params['popen'] = options.popen
    

    if options.pextend:
        params['pextend'] = options.pextend
