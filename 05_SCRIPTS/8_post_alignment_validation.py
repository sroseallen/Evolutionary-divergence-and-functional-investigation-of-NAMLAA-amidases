...
seqs = list(
    SeqIO.parse(
        "./01_DATA/Amidase_3/04_Multiple_Alignment/Validation/candidate_validation_set.txt",
        "fasta",
    )
)
strucs = list(
    SeqIO.parse(
        "./01_DATA/Amidase_3/04_Multiple_Alignment/Validation/candidate_USalign_validation_set.txt",
        "fasta-2line",
    )
)
...
def validate_msa(A, B):
    for record in seqs:
        if A in record.id:
            msa1_temp = record.seq
        if B in record.id:
            msa2_temp = record.seq
    ...
    for record in strucs:
        if record.id == str(A + B + "USalign"):
            struc1 = record.seq
        if record.id == str(B + A + "USalign"):
            struc2 = record.seq
...
tpr = pd.DataFrame(index=structures, columns=structures)
ppv = pd.DataFrame(index=structures, columns=structures)
