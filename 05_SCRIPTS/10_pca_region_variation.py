...
for i in range(len(PC_numbers)):
    plt.text(i+1, prop_var[i]+0.01, f"{prop_var[i]*100:.1f}%", ha="center", va="center")
...
sns_ax = sns.scatterplot(x="PC1",y="PC3",hue="label",data=pca_output,palette="Accent")
sns.move_legend(sns_ax, "upper left", bbox_to_anchor=(1, 1))
plt.rcParams["axes.labelsize"] = 15
...
sns_ax = sns.scatterplot(x="PC2",y="PC3",hue="label",data=pca_output,palette="Accent")
sns.move_legend(sns_ax, "upper left", bbox_to_anchor=(1, 1))
plt.rcParams["axes.labelsize"] = 15
...
def biplot(score, components, labels=None):
    ...
    plt.scatter(x_score * scalex, 
                y_score * scaley, 
                s=5, color="gray")
    for i in range(features):
        plt.arrow(0, # start/end positions for the arrow
                  0, 
                  components[i,0], 
                  components[i,1],
                  head_width=0.05, head_length=0.05, overhang=1,
                  color = "red")
        plt.text(components[i,0] * 0.7, # place label text slightly offset from the end of the arrow
                 components[i,1] * 0.7, 
                 labels[i], 
                 color = "maroon")
