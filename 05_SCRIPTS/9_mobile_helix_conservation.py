...
def plot_insertion(
    ...
    if write == True:
        df.to_csv(
            f"./01_DATA/Amidase_3/05_1_Region_Conservation/{start}-{end}.csv",
            index=False,
        )

    # reformat for plotting: gram staining status
    df_grouped = (
        df.groupby(["occupancy", "gram_status"]).size().reset_index(name="count")
    )
    ...
    for num in range((end - start) + 2):
        df_grouped = pd.concat(
            [
                df_grouped,
                pd.DataFrame([{"occupancy": num, "gram_status": "NA", "count": 0}]),
            ]
        )
    df_plotting = df_grouped.pivot_table(
        index="occupancy", columns="gram_status", values="count", fill_value=0
    ).reset_index()
...
plot_insertion(
    1, 6, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I1_region.png"
)  # I-1
print("Now plotting region 2...")
plot_insertion(
    15, 57, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I2_region.png"
)  # I-2
print("Now plotting region 3...")
plot_insertion(
    81, 85, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I3_region.png"
)  # I-3
print("Now plotting region 4...")
plot_insertion(
    96, 128, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I4_region.png"
)  # I-4
print("Now plotting region 5...")
plot_insertion(
    154, 175, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I5_region.png"
)  # I-5
print("Now plotting region 6...")
plot_insertion(
    184,
    241,
    save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I6_region.png",
    write=True,
)  # I-6
...
plot_insertion(
    252,
    303,
    save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I7_region.png",
    write=True,
)  # I-7
print("Now plotting region 8...")
plot_insertion(
    355, 399, save_path="./01_DATA/Amidase_3/05_1_Region_Conservation/I8_region.png"
)  # I-8
print("Plots saved!")
...
with open(
    "./01_DATA/Amidase_3/05_1_Region_Conservation/fingerprints.json", "w"
) as file:
...
with open(
    "./01_DATA/Amidase_3/05_1_Region_Conservation/fingerprints.json", "r"
) as file:
...
for file_name in os.listdir("./01_DATA/Amidase_3/05_1_Region_Conservation/"):
    if file_name.startswith("["):
        random_selection = fingerprint_selection(
...
filtered_alignment = [
    record
    for record in alignment
    if record.id.split(":")[1] in list(taxid_lookup.keys())
]
...
AlignIO.write(
    filtered_alignment_nodup,
    "./01_DATA/Amidase_3/06_Phylogeny/representative_alignment.fasta",
    "fasta",
)

# prepare the sequences for phylogeny (phylip format)
filtered_alignment_nodup = AlignIO.read(
    "./01_DATA/Amidase_3/06_Phylogeny/representative_alignment.fasta", "fasta"
)
unique_names = set()
...
AlignIO.write(
    filtered_alignment_nodup,
    "./01_DATA/Amidase_3/06_Phylogeny/representative_phylip.phy",
    "phylip",
)
...
for file_name in os.listdir("./01_DATA/Amidase_3/05_1_Region_Conservation/"):
    if file_name.startswith("["):
        with open(
            f"./01_DATA/Amidase_3/05_1_Region_Conservation/{file_name}", "r"
        ) as file:
            fingerprints = json.load(file)
