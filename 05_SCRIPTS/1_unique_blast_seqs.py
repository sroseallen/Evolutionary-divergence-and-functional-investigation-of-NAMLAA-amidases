...
for file_name in os.listdir("./01_DATA/Amidase_3/03_1_Sequence_Searches/"):
    if file_name.endswith("seqdump.txt"):
        input.append(
            os.path.join("./01_DATA/Amidase_3/03_1_Sequence_Searches/", file_name)
        )
...
with open(
    "./01_DATA/Amidase_3/03_1_Sequence_Searches/BLAST_all_seq_nospace.txt", "w"
) as f:
    f.write(spaceloss)
...
for record in SeqIO.parse(
    "./01_DATA/Amidase_3/03_1_Sequence_Searches/BLAST_all_seq_nospace.txt", "fasta"
):
...
for idrec, seq in unique_seqs.items():
    ...
    all.append(record)
with open(
    "./01_DATA/Amidase_3/03_1_Sequence_Searches/BLAST_all_seq_unique.txt", "w"
) as output:
