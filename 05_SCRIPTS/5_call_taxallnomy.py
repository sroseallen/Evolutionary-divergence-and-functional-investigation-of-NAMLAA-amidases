...
with open(
    "./01_DATA/Amidase_3/03_2_Sequence_Annotation/taxallnomy_input.txt", "r"
) as tax:
...
for id in tqdm(tax_ids_all):
    call = requests.get(
        f"http://bioinfo.icb.ufmg.br/cgi-bin/taxallnomy/taxallnomy_multi.pl?txid={id}&rank=main&format=json"
    )
    ...
    try:
        ...
        new_id = (
            ...
            + f"{id}"
            + ":"
            + (call_json[f"{id}"]["phylum"])
            + ":"
            + (call_json[f"{id}"]["family"])
            + ":"
            + (call_json[f"{id}"]["species"])
            + ","
        )
    except:
        new_id = ">" + f"Nil, {id}" + ","
    ...
    with open(
        "./01_DATA/Amidase_3/03_2_Sequence_Annotation/familyID_temp.txt", "a"
    ) as addfam:
        addfam.write(new_id)
...
with open(
    ...
    for line in f:
        if line.startswith(">"):
            ...
            counter += 1
        with open(
            "./01_DATA/Amidase_3/03_2_Sequence_Annotation/all_seq_taxid.txt", "a"
        ) as add:
            add.write(line)

count = 0
with open(
    "./01_DATA/Amidase_3/03_2_Sequence_Annotation/all_seq_taxid.txt", "r"
) as file:
    for line in file:
        if line.startswith(">"):
            count += 1
